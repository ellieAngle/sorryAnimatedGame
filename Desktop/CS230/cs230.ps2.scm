;PARTNERS: Ellie Angle ega12, Kate Straneva kms190
  ...
  (lambda ((lst <list>) (key <key>))
    (let ((n (key-modulus key)))
      (let loop ((rst lst) (prev 0))
        (cond ((null? rst) rst)
              (else
               (let ((cur (modulo (- (rsa-transform (modulo (car rst) n) key) prev) n)))
                 (cons cur (loop (cdr rst) (car rst))))))))))
...
(define encrypt-and-sign
  (lambda ((lst <list>) (key-pub <key>) (key-pri <key>))
    (let ((crypt (rsa-encrypt lst key-pub)))
      (cons crypt (rsa-transform (compress crypt key-pri) key-pri)))))
; PROBLEM 2 FUNCTION
(define decrypt-and-verify
  (lambda ((lst <list>) (key-pri <key>) (key-pub <key>))
    (let ((comp-h (compress (car lst) key-pub)))
      (cond ((= comp-h (rsa-transform (cdr lst) key-pub)) (rsa-decrypt (car lst) key-pri))
            (else
             #f)))))
; PROBLEM 3 FUNCTION
(define crack-rsa
  (lambda ((key-pub <key>))
    (let ((n (key-modulus key-pub))
          (e (key-exponent key-pub)))
...
(define pollard
  (lambda ((n <integer>) (base <integer>) (max <integer>))
    (letrec ((find-factor
              (lambda ((k <integer>) (base <number>))
                (let ((r (exptmod base k n)))
                  (let ((d (caddr (euclid (- r 1) n))))
                    (cond ((= k max) #f)
                          ((or (= d 1) (= d n))
                           (find-factor (+ k 1) r))
                          (else
...
;;-----------------------------------------------------------------------------------------;;
#|
Problem 1 (code located above)
(define result-1
  (rsa-encrypt "test message" (key-pair-public test-key-1)))
(define myTest-11
  (rsa-encrypt "ellie and kate are super cool and dope!" (key-pair-public test-key-2)))
(define myTest-12
  (rsa-encrypt "today for lunch i ate the hearts of my enemies and drank the tears of my h8ers" (key-pair-public test-key-1)))

(rsa-decrypt result-1 (key-pair-private test-key-1))
--> "test message"
(rsa-decrypt myTest-11 (key-pair-private test-key-2))
--> "ellie and kate are super cool and dope!"
(rsa-decrypt myTest-12 (key-pair-private test-key-1))
--> "today for lunch i ate the hearts of my enemies and drank the tears of my h8ers"
|#
;;-----------------------------------------------------------------------------------------;;
#|
Problem 2 (code located above)
(define result-2
     (encrypt-and-sign "Test message from User 1 to User 2"
                       (key-pair-public test-key-2)
                       (key-pair-private test-key-1)))
(define myTest-21
  (encrypt-and-sign "Netflix and chill?"
                    (key-pair-public test-key-1)
                    (key-pair-private test-key-2)))
(define myTest-22
  (encrypt-and-sign "If this works, we're done for"
                    (key-pair-public test-key-1)
                    (key-pair-private test-key-2)))
...
(decrypt-and-verify myTest-21
                    (key-pair-private test-key-1)
                    (key-pair-public test-key-2))
--> "Netflix and chill?"
(decrypt-and-verify myTest-22
                    (key-pair-private test-key-2)
                    (key-pair-public test-key-1))
--> #f
|#
...
#|
Problem 4
Joyce sent the message to Vinit
the secret message is: "If cryptography is outlawed, only outlaws will have cryptography."

Procs used:
(rsa-decrypt (signed-message-body secret-message) (crack-rsa Vinit))
^^ first found the receiver of the message by seeing who's private key returned a message

(decrypt-and-verify secret-message (crack-rsa Vinit) Joyce)
^^ Then, knowing Vinit received the message, we used decrypt-and-verify to figure out who
sent the message (i.e. who's public key decrypted the encrypted signature)
|#
;;-----------------------------------------------------------------------------------------;;
#|
Problem 5 (code located above)
